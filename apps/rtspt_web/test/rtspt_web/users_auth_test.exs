defmodule RtsptWeb.UsersAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, RtsptWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})

    %{users: %{users_fixture() | authenticated_at: NaiveDateTime.utc_now(:second)}, conn: conn}
  end
  ...
  describe "log_in_users/3" do
    ...
    test "redirects to dashboard when users is already logged in", %{conn: conn, users: users} do
      conn =
        conn
        |> assign(:current_scope, Scope.for_users(users))
        |> UsersAuth.log_in_users(users)
  ...
  describe "logout_users/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      RtsptWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_sudo_mode" do
    ...
    test "redirects when authentication is too old", %{conn: conn, users: users} do
      ...
      assert NaiveDateTime.compare(token_inserted_at, users.authenticated_at) == :gt
      session = conn |> put_session(:users_token, users_token) |> get_session()
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        RtsptWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
