defmodule Rtspt.AccountsTest do
  use Rtspt.DataCase

  alias Rtspt.Accounts

  import Rtspt.AccountsFixtures
  alias Rtspt.Accounts.{Users, UsersToken}
  ...
  describe "get_users!/1" do
    test "raises if id is invalid" do
      assert_raise Ecto.NoResultsError, fn ->
        Accounts.get_users!("11111111-1111-1111-1111-111111111111")
      end
    end
  ...
  describe "sudo_mode?/2" do
    test "validates the authenticated_at time" do
      ...
      assert Accounts.sudo_mode?(%Users{authenticated_at: NaiveDateTime.utc_now()})
      assert Accounts.sudo_mode?(%Users{authenticated_at: NaiveDateTime.add(now, -19, :minute)})
      refute Accounts.sudo_mode?(%Users{authenticated_at: NaiveDateTime.add(now, -21, :minute)})

      # minute override
      refute Accounts.sudo_mode?(
               %Users{authenticated_at: NaiveDateTime.add(now, -11, :minute)},
               -10
             )
  ...
  describe "generate_users_session_token/1" do
    ...
    test "duplicates the authenticated_at of given users in new token", %{users: users} do
      users = %{users | authenticated_at: NaiveDateTime.add(NaiveDateTime.utc_now(:second), -3600)}
      token = Accounts.generate_users_session_token(users)
      assert users_token = Repo.get_by(UsersToken, token: token)
      assert users_token.authenticated_at == users.authenticated_at
      assert NaiveDateTime.compare(users_token.inserted_at, users.authenticated_at) == :gt
    end
  end
