defmodule Rtspt.Accounts.UsersToken do
  use Ecto.Schema
  import Ecto.Query
  alias Rtspt.Accounts.UsersToken
  ...
  schema "users_tokens" do
    ...
    belongs_to :users, Rtspt.Accounts.Users
  ...
  def build_session_token(users) do
    ...
    dt = users.authenticated_at || NaiveDateTime.utc_now(:second)
  ...
  The query returns the users found by the token, if any, along with the token's creation time.
  ...
  def verify_session_token_query(token) do
    query =
      from token in by_token_and_context_query(token, "session"),
        ...
        select: {%{users | authenticated_at: token.authenticated_at}, token.inserted_at}
  ...
  the token in the application to gain access. Furthermore, if the users changes
  ...
  If found, the query returns a tuple of the form `{users, token}`.
      ...
      {:ok, decoded_token} ->
        ...
        query =
          from token in by_token_and_context_query(hashed_token, "login"),
            ...
            where: token.sent_to == users.email,
            select: {users, token}
  ...
  The query returns the users_token found by the token, if any.
