defmodule Rtspt.Accounts do
  ...
  alias Rtspt.Accounts.{Users, UsersToken, UsersNotifier}
  ...
  def sudo_mode?(%Users{authenticated_at: ts}, minutes) when is_struct(ts, NaiveDateTime) do
  ...
  See `Rtspt.Accounts.Users.email_changeset/3` for a list of supported options.
  ...
  See `Rtspt.Accounts.Users.password_changeset/3` for a list of supported options.
  ...
  ## Examples
      ...
      iex> deliver_users_update_email_instructions(users, current_email, &url(~p"/users/settings/confirm-email/#{&1}"))
      {:ok, %{to: ..., body: ...}}
  ...
  @doc """
  Delivers the magic link login instructions to the given users.
  """
  def deliver_login_instructions(%Users{} = users, magic_link_url_fun)
